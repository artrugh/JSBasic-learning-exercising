...
const array = [3, 45, 6, 56, 7, 9].map(x => x + 1);
//console.log(array);
...
let sum = [1, 2, 3, 4, 5].reduce((acc, num) => acc + num);
//console.log(sum);
...
const data = ['car', 'car', 'truck', 'truck', 'bike', 'walk',
    'car', 'van', 'bike', 'walk', 'car', 'van', 'car', 'truck', 'pogostick'];
...
//     if (data[i] === "bike") { object.bike += 1 }
//     else if (data[i] === "car") { object.car += 1 }
//     else if (data[i] === "pogostick") { object.pogostick += 1 }
//     else if (data[i] === "truck") { object.truck += 1 }
//     else if (data[i] === "van") { object.van += 1 }
//     else if (data[i] === "walk") { object.walk += 1 }
// }

data.forEach(i => {
    if (i === "bike") { return object.bike += 1 }
    else if (i === "car") { return object.car += 1 }
    else if (i === "pogostick") { return object.pogostick += 1 }
    else if (i === "truck") { return object.truck += 1 }
    else if (i === "van") { return object.van += 1 }
    else if (i === "walk") { return object.walk += 1 }
...
var names = inventors.map(obj => obj.first + " " + obj.last);
//console.log(names);

// Reduce
// How many years did all of the inventors live?
var years = inventors.reduce((acc, num) => acc + (num.passed - num.year))
//console.log(years);


// 5. Square Root
// Given an array of numbers, find the square root of those numbers using map.
const arr = [2, 4, 5].map(x => Math.sqrt(x, 2))
//console.log(arr);
...
const hallo = "hallo";
const numChar = str => [...str].length;

//console.log(numChar(hallo));
...
let filterEvens = [1, 2, 3, 11, 12, 13].filter(x => x % 2 === 0); //returns [2,12]
...
const niks = ["Rika", "Jacob", "Alex", "Oliver", "Marika"];
//console.log(filterItems(niks, 'ka'));
...
// let newArray = originalArray.map(num => `{${num.key}: "${num.name}"}`)
let newArray = originalArray.filter(num => num.key + num.name)
...
// ```javascript
let arrayOfIntegers = [4, 200, 90, 30, 12, 75, 21, 3].sort((a, b) => a - b)
