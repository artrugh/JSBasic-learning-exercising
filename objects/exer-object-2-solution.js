// Arrow Functions Exercises
// Complete the following tasks using arrow functions.

// 1. Multiply
// Multiply two numbers and return the result using an arrow function.

const mult = (num1, num2) => [num1 * num2]
//console.log(mult(2, 4));

// 

// const o = { p: 42, q: false };
// const { p: foo, q: baz } = o
// console.log(foo); //42
// console.log(baz); //false
// console.log(o.p); //42



// 2. Double The Odds
// Given any array of numbers e.g. [3, 5, 67, 12, 34, 4, 9, 30], filter the odd numbers into a new array and multiply each odd number by 2.


// function oddNum(value) {
//  value % 2 === 0
// }

// var filtered = [3, 5, 67, 12, 34, 4, 9, 30].filter(oddNum).map(x => x * 2);

var filtered = [3, 5, 67, 12, 34, 4, 9, 30].filter(value => value % 2 !== 0).map(x => x * 2);

// console.log(filtered);



// 3. Loopyloop
// Given an array of numbers, loop through the array and subract 10 from each element using an arrow function. Return the new array.
let arr = [3, 5, 67, 12, 34, 4, 9, 30];

const newarr = arr.map(x => x - 10);


//console.log(newarr);


// 4. Cars
// Create an object named car. Create key pair values which include the model of the car, the manufacturer, year etc. Create an arrow function which prints the key pair values of the object.

const car = [
    { model: "56", manufacturer: "ford", year: 1992 },
    { model: "43", manufacturer: "chevrolet", year: 1994 }
]

// const getCars = autos => autos.forEach(car => console.log(car));
// getCars(cars);

car.find(function (value) {
    console.log(value);
});

console.log(Object.values(car));



// 5. Filter
// Given this array: , use the array filter method and an arrow function to create an array of the numbers greater than 70. Store the result into a variable.


// function greaterThan(value) {
//     return value > 70
// }


var greater = [3, 62, 234, 7, 23, 74, 23, 76, 92].filter(value => value > 70)
//console.log(greater);


// 6. Geometrizer
// Create two functions that calculate the properties of a circle.

// Create a function called calcCircumfrence:
// Pass the radius to the function.
// Calculate the circumference based on the radius, and output "The circumference is NN".

// Create a function called calcArea:
// Pass the radius to the function.
// Calculate the area based on the radius, and output "The area is NN".

const calcCir = radium => `The circumference is ${2 * 3.14 * radium}cm.`;

const calcArea = radium => `The area is ${Math.pow(radium, 2) * 3.14}cm.`;

// console.log(calcCir(3), calcArea(3));


// 7. The Inventors
// Given the following array, complete the tasks below.

const inventors = [{
    first: 'Albert',
    last: 'Einstein',
    year: 1879,
    passed: 1955
},
{
    first: 'Isaac',
    last: 'Newton',
    year: 1643,
    passed: 1727
},
{
    first: 'Galileo',
    last: 'Galilei',
    year: 1564,
    passed: 1642
},
{
    first: 'Marie',
    last: 'Curie',
    year: 1867,
    passed: 1934
},
{
    first: 'Johannes',
    last: 'Kepler',
    year: 1571,
    passed: 1630
},
{
    first: 'Nicolaus',
    last: 'Copernicus',
    year: 1473,
    passed: 1543
},
{
    first: 'Max',
    last: 'Planck',
    year: 1858,
    passed: 1947
},
{
    first: 'Katherine',
    last: 'Blodgett',
    year: 1898,
    passed: 1979
},
{
    first: 'Ada',
    last: 'Lovelace',
    year: 1815,
    passed: 1852
},
{
    first: 'Sarah E.',
    last: 'Goode',
    year: 1855,
    passed: 1905
},
{
    first: 'Lise',
    last: 'Meitner',
    year: 1878,
    passed: 1968
},
{
    first: 'Hanna',
    last: 'Hammarström',
    year: 1829,
    passed: 1909
}
];
// Filter the list of inventors for those who were born in the 1500's.
// Sort the inventors by birthdate, oldest to youngest.
// Return an array of the inventors' first and last names.


//console.log(inventors.filter(obj => obj.year >= 1500 && obj.year <= 1599));


//console.log(inventors.sort((a, b) => b.year - a.year));

let names = [];

// for (let i in inventors) {

//     names.push(inventors[i].first + inventors[i].last)
// }

inventors.forEach(obj => names.push(obj.first + obj.last))

console.log(names)




